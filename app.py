...
/* Chat takes full height, no fixed scroll — auto scrolls naturally */
.chat-area {
    margin-bottom: 12px;
}

/* Anchor div at bottom for auto scroll */
...
// Also run after short delay to catch dynamic content
...
# ── Chat display ──────────────────────────────────────────────────────────────
chat_html = '<div class="chat-area"><div class="chat-wrapper">'
...
# Anchor at the very bottom for auto-scroll
chat_html += '</div><div id="chat-bottom"></div></div>'
...
with col_input:
    user_input = st.text_input(
        ...
        key=f"user_input_{st.session_state.input_key}",   # key change clears the box
...
if send and user_input.strip():
    ...
    # Increment key to clear the input box on next render
    st.session_state.input_key += 1
